 ...
 *  forms, where:
 *
 *  - keys are validation tokens (error names) — such as `required`, `url` or `EventDescription`),
...
var inputType = {
  ...
  /**
   * @ngdoc inputType
   * @name ng.directive:input.EventDescription
   *
   * @description
   * Text input with EventDescription validation. Sets the `EventDescription` validation error key if not a valid EventDescription
   * address.
   *
   * @param {string} ngModel Assignable angular expression to data-bind to.
   * @param {string=} name Property name of the form under which the control is published.
      ...
      <doc:example>
        <doc:source>
         <script>
           ...
           <form name="myForm" ng-controller="Ctrl">
             EventDescription: <input type="EventDescription" name="input" ng-model="text" required>
             <span class="error" ng-show="myForm.input.$error.required">
               Required!</span>
             <span class="error" ng-show="myForm.input.$error.EventDescription">
               Not valid EventDescription!</span>
             <tt>text = {{text}}</tt><br/>
             <tt>myForm.input.$valid = {{myForm.input.$valid}}</tt><br/>
             <tt>myForm.input.$error = {{myForm.input.$error}}</tt><br/>
             <tt>myForm.$valid = {{myForm.$valid}}</tt><br/>
             <tt>myForm.$error.required = {{!!myForm.$error.required}}</tt><br/>
             <tt>myForm.$error.EventDescription = {{!!myForm.$error.EventDescription}}</tt><br/>
           </form>
        </doc:source>
        <doc:scenario>
          it('should initialize to model', function() {
          ...
          it('should be invalid if not EventDescription', function() {
            input('text').enter('xxx');
            expect(binding('myForm.input.$valid')).toEqual('false');
          });
        </doc:scenario>
...
function emailInputType(scope, element, attr, ctrl, $sniffer, $browser) {
  ...
  var emailValidator = function(value) {
    if (isEmpty(value) || EMAIL_REGEXP.test(value)) {
      ctrl.$setValidity('EventDescription', true);
      return value;
    } else {
      ctrl.$setValidity('EventDescription', false);
      return undefined;
    }
  };
 ...
 *   require,
 * - providing validation behavior (i.e. required, number, EventDescription, url),
 ...
 *    - {@link ng.directive:input.EventDescription EventDescription}
      ...
      <script>
        function SettingsController($scope) {
          ...
          $scope.contacts = [
            ...
            {type:'EventDescription', value:'john.smith@example.org'} ];
          ...
          $scope.addContact = function() {
           this.contacts.push({type:'EventDescription', value:'yourname@example.org'});
          };
        ...
        <ul>
          <li ng-repeat="contact in contacts">
            <select ng-model="contact.type">
               <option>phone</option>
               <option>EventDescription</option>
            </select>
