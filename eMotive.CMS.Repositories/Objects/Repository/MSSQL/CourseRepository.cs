            ...
            using (var cn = Connection)
            {
                using (var transaction = new TransactionScope())
                {
                    ...
                    if (course.CourseYears.HasContent())
                    {
                        ...
                        sql =
                            "SET IDENTITY_INSERT CourseYears ON;INSERT INTO CourseYears (ID, Name, Abbreviation, YearStart, Year, CourseID) Values (@ID, @Name, @Abbreviation, @YearStart, @Year, @CourseID);SET IDENTITY_INSERT CourseYears OFF;";
